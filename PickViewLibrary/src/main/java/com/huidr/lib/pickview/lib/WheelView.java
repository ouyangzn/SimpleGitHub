package com.huidr.lib.pickview.lib;
...
import com.huidr.lib.pickview.R;
import com.huidr.lib.pickview.adapter.WheelAdapter;
import com.huidr.lib.pickview.listener.OnItemSelectedListener;
...
public class WheelView extends View {
  ...
  // 离左右的距离
  private final static int DEFAULT_MARGIN_LEFT_RIGHT = 2;
  Context context;
  Handler handler;
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    //            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET - drawCenterContentStart, centerY, paintCenterText);
    //
    //        }
    counter = 0;
    while (counter < itemsVisible) {
      ...
      if (angle >= 90F || angle <= -90F) {
        ...
        String content = contentText + label;
        measuredCenterContentStart(content);
        measuredOutContentStart(content);
        float translateY =
        ...
        if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
          ...
          canvas.drawText(content, drawOutContentStart, maxTextHeight, paintOuterText);
          canvas.restore();
          canvas.save();
          canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int) (itemHeight));
          canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
          canvas.drawText(content, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET,
              paintCenterText);
          canvas.restore();
    ...
    //            //靠右并留出空隙
    ////            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET, centerY, paintCenterText);
    //            float startX = drawRightContentStart + CENTERCONTENTOFFSET - drawCenterContentStart;
    //            canvas.drawText(label, startX, centerY, paintCenterText);
    //            Log.d("WheelView", "---------- startX = " + startX);
    //
    //        }
  }
  ...
  private void measuredCenterContentStart(String content) {
    ...
    switch (mGravity) {
      case Gravity.CENTER:
        ...
        break;
      case Gravity.LEFT:
        drawCenterContentStart = DEFAULT_MARGIN_LEFT_RIGHT;
        break;
      case Gravity.RIGHT:
        drawCenterContentStart = measuredWidth - rect.width() - DEFAULT_MARGIN_LEFT_RIGHT;
        break;
    }
  }
  ...
  private void measuredOutContentStart(String content) {
    ...
    switch (mGravity) {
      case Gravity.CENTER:
        ...
        break;
      case Gravity.LEFT:
        drawOutContentStart = DEFAULT_MARGIN_LEFT_RIGHT;
        break;
      case Gravity.RIGHT:
        drawOutContentStart = measuredWidth - rect.width() - DEFAULT_MARGIN_LEFT_RIGHT;
        break;
    }
  }
