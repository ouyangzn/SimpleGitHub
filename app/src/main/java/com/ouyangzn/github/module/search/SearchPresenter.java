...
/**
 * Created by ouyangzn on 2017/6/9.<br/>
 * Description：
 */
public class SearchPresenter extends ISearchPresenter {
  ...
  public SearchPresenter(Context context, LifecycleProvider<FragmentEvent> provider) {
    mProvider = provider;
  ...
  @Override public void saveSearchFactor(SearchFactor factor) {
    Observable.just(factor)
        ...
        .doOnNext(factor1 -> mConfigSp.edit()
            .putString(KEY_LANGUAGE, App.getApp().getGson().toJson(factor1))
            .apply())
        .subscribe();
  }
